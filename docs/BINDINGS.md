...
`:JSON [action] [args] [flags]` — range-aware (no range = whole buffer).

| Invocation | Range | Args | Flags | Description |
| --- | --- | --- | --- | --- |
| `:JSON` | — | — | — | Default action: same as `:JSON pretty`. |
| `:JSON pretty [indent]` | yes | `indent?: INT` | — | Pretty-print (default 2-space indent). |
| `:JSON compact` | yes | — | — | Collapse onto one line. |
| `:JSON lines [--sep=X]` | yes | — | `--sep: STRING` | One `path: value` per leaf, nested keys dotted. |
| `:JSON keys [--sep=X]` | yes | — | `--sep: STRING` | Only the (dotted) key paths, no values. |
| `:JSON sort [indent]` | yes | `indent?: INT` | — | Pretty-print with object keys sorted (see `docs/commands.md`). |
...
None — `data.nvim` only acts on an explicit `:JSON` invocation.
